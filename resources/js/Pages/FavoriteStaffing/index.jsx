...
export default function Index({ jobs, filters, statuses, job_classes }) {
    const [search, setSearch] = useState(filters.search || '');
    const [status, setStatus] = useState(filters.status || '');
    const [job_class, setJobClass] = useState(filters.job_class || '');
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            router.get(route('favorite-staffing.index'), {
                search,
                status,
                job_class
            }, {
    ...
    return (
        <AuthenticatedLayout
            header="Favorite Staffing Job Listings"
        >
            <Head title="Favorite Staffing Jobs" />
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-zinc-400 mt-1">
                            Showing {jobs.from}-{jobs.to} of {jobs.total} jobs from Favorite Staffing.
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="Search job class, location, ID..."
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-zinc-800 bg-zinc-900/50">
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-400">Timeline</th>
                                    <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-zinc-400">Details</th>
                            ...
                            <tbody className="divide-y divide-zinc-800">
                                {jobs.data.map((job) => (
                                    <tr key={job.id} className="hover:bg-zinc-900/40 transition-colors group">
                                        ...
                                        <td className="px-6 py-5">
                                            <Link 
                                                href={route('all-jobs.show', { portal: 'Favorite Staffing', id: job.order_id })}
