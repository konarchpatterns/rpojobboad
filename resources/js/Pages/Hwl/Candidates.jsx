...
import { Search, Filter, User, Mail, Phone, MapPin, Calendar, ArrowLeft } from 'lucide-react';

export default function Candidates({ candidates, filters, genders, states }) {
    ...
    const handleFilter = () => {
        router.get(route('hwl.candidates'), {
            search: searchTerm,
            gender: genderFilter,
            state: stateFilter,
        }, {
    ...
    useEffect(() => {
        if (genderFilter !== (filters.gender || '') || stateFilter !== (filters.state || '')) {
            handleFilter();
        }
            ...
            header={
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="font-semibold text-xl text-zinc-100 leading-tight">HWLMSP Candidates</h2>
                    </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-zinc-800 bg-zinc-900/30">
                                <th className="px-6 py-4 text-sm font-semibold text-zinc-400">Profile ID</th>
                                ...
                                <th className="px-6 py-4 text-sm font-semibold text-zinc-400">Gender/DOB</th>
                                    ...
                                    <tr key={candidate.id} className="hover:bg-zinc-800/20 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-sm text-zinc-400">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {candidate.city ? `${candidate.city}, ${candidate.state}` : candidate.state || 'N/A'}
                                            </div>
                                            {candidate.zip_code && <div className="text-xs text-zinc-500 ml-5">{candidate.zip_code}</div>}
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="text-sm text-zinc-300">{candidate.gender || 'N/A'}</div>
