...
import { Search, Filter, User, Mail, Phone, MapPin, ArrowLeft } from 'lucide-react';

export default function Candidates({ candidates, filters, statuses, professions }) {
    const [searchTerm, setSearchTerm] = useState(filters.search || '');
    const [statusFilter, setStatusFilter] = useState(filters.status || '');
    const [professionFilter, setProfessionFilter] = useState(filters.profession || '');

    const handleFilter = () => {
        router.get(route('laboredge.candidates'), {
            search: searchTerm,
            status: statusFilter,
    ...
    useEffect(() => {
        if (statusFilter !== (filters.status || '') || professionFilter !== (filters.profession || '')) {
            handleFilter();
        }
    ...
    return (
            ...
            header={
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <h2 className="font-semibold text-xl text-zinc-100 leading-tight">National Staffing Candidates</h2>
                    </div>
        ...
        >
            <Head title="National Staffing Candidates" />
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-zinc-800 bg-zinc-900/30">
                                ...
                                <th className="px-6 py-4 text-sm font-semibold text-zinc-400">Profession/Specialty</th>
                            ...
                            {candidates.data.length > 0 ? (
                                candidates.data.map((candidate) => (
                                    <tr key={candidate.id} className="hover:bg-zinc-800/20 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="text-sm text-zinc-300">{candidate.profession || 'N/A'}</div>
                                            <div className="text-xs text-zinc-500">{candidate.specialty || 'N/A'}</div>
                                        </td>
